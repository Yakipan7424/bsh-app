    ...
    <footer className="pointer-events-none fixed bottom-0 left-1/2 z-40 w-[min(89%,22rem)] max-w-sm -translate-x-1/2 isolate pt-0.5 pb-[max(7px,env(safe-area-inset-bottom,0px))]">
      ...
      */}
      <nav
        className={
          isV2
            ? "pointer-events-auto relative flex min-h-[1.85rem] items-center justify-between rounded-full border border-bsh-gold/35 bg-gradient-to-r from-bsh-burgundy via-bsh-bordeaux/90 to-bsh-burgundy px-2 py-0.5 shadow-[0_10px_28px_-10px_rgba(0,0,0,0.65)] transition-colors duration-300 ease-out"
      ...
      >
        ...
        <Link
          ...
          className={
            isV2
              ? "absolute left-1/2 top-1/2 z-20 flex min-h-8 min-w-8 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-bsh-gold/70 bg-bsh-graphite/90 p-0.5 shadow-[0_0_0_1px_rgba(107,31,46,0.5)] transition-all duration-300 ease-out active:opacity-85"
